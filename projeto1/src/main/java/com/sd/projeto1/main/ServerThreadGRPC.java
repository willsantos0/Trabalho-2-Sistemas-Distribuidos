...
public class ServerThreadGRPC implements Runnable {
	...
	public void run() {
		...
		try {
			...
			Server server = ServerBuilder.forPort( settings.getPortGRPC()).addService( new com.sd.projeto1.service.ContextService( logQueue, executeQueue, context, observers ) ).build();
